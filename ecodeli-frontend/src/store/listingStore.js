...
const useListingStore = create((set, get) => ({
  ...
  fetchListingById: async (id) => {
    ...
    try {
      ...
      // S'assurer que nous prenons la propriété data si elle existe
      const actualData = response.data || response;
      
      console.log("Détails de l'annonce reçus de l'API:", actualData);
      
      // S'assurer que nous avons toutes les données nécessaires
      set({ currentListing: actualData, isLoading: false });
      return actualData;
    } catch (error) {
